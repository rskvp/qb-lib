...
import (
	...
	token2 "github.com/rskvp/qb-lib/qb_auth0/jwt/elements"
...
func TestRSAPSSVerify(t *testing.T) {
	...
	if rsaPSSKey, err = signing.ParseRSAPublicKeyFromPEM(key); err != nil {
...
func TestRSAPSSSign(t *testing.T) {
	...
	if rsaPSSKey, err = signing.ParseRSAPrivateKeyFromPEM(key); err != nil {
...
func TestRSAPSSSaltLengthCompatibility(t *testing.T) {
	...
	ps256SaltLengthEqualsHash := &signing.SigningMethodRSAPSS{
		SigningMethodRSA: signing.SigningMethodPS256.SigningMethodRSA,
		Options: &rsa.PSSOptions{
			SaltLength: rsa.PSSSaltLengthEqualsHash,
		},
	}

	ps256SaltLengthAuto := &signing.SigningMethodRSAPSS{
		SigningMethodRSA: signing.SigningMethodPS256.SigningMethodRSA,
		Options: &rsa.PSSOptions{
			SaltLength: rsa.PSSSaltLengthAuto,
		},
	}
	if !verify(signing.SigningMethodPS256, makeToken(ps256SaltLengthEqualsHash)) {
		t.Error("SigningMethodPS256 should accept salt length that is defined in RFC")
	}
	if !verify(ps256SaltLengthEqualsHash, makeToken(signing.SigningMethodPS256)) {
		t.Error("Sign by SigningMethodPS256 should have salt length that is defined in RFC")
	}
	if !verify(signing.SigningMethodPS256, makeToken(ps256SaltLengthAuto)) {
		t.Error("SigningMethodPS256 should accept auto salt length to be compatible with previous versions")
	}
	if !verify(ps256SaltLengthAuto, makeToken(signing.SigningMethodPS256)) {
...
func makeToken(method commons.SigningMethod) string {
	token := jwt.NewWithClaims(method, token2.StandardClaims{
		Issuer:   "example",
...
func verify(signingMethod commons.SigningMethod, token string) bool {
