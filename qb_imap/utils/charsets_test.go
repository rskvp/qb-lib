...
func TestInvalidCharsetReader(t *testing.T) {
	...
	outputReader, err := utils.NewCharsetReader("INVALIDcharsetZZZ", inputReader)
	if outputReader != nil {
...
func TestCharsetReader(t *testing.T) {
	...
	for _, tt := range testTable {
		...
		outputReader, err := utils.NewCharsetReader(tt.charset, inputReader)
		if err != nil {
...
func TestFindCharsetInHTML(t *testing.T) {
	...
	for _, tt := range ttable {
		got := utils.FindCharsetInHTML(tt.input)
		if got != tt.want {
...
func TestConvertToUTF8String(t *testing.T) {
	...
	for _, v := range testTable {
		s, err := utils.ConvertToUTF8String(v.charset, v.input)
		if err != nil {
	...
	_, err := utils.ConvertToUTF8String("123", []byte("there is no 123 charset"))
	if err == nil {
