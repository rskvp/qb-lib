...
func ReadEnvelope(r io.Reader) (*Envelope, error) {
	...
	if err != nil {
		return nil, qbc.Errors.WithMessage(err, "Failed to ReadParts")
	}
...
func EnvelopeFromPart(root *Part) (*Envelope, error) {
	...
	if e.Text == "" && e.HTML != "" {
		...
		var err error
		if e.Text, err = utils.Html2TextFromString(e.HTML); err != nil {
...
func parseTextOnlyBody(root *Part, e *Envelope) error {
	...
	if isHTML {
		...
		if charset == "" {
			...
			if charset = utils.FindCharsetInHTML(rawHTML); charset != "" {
				// Found charset in HTML
				if convHTML, err := utils.ConvertToUTF8String(charset, root.Content); err == nil {
