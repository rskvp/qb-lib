...
func (p *Part) setupMIMEHeaders() transferEncoding {
	...
	if p.FirstChild != nil && p.Boundary == "" {
		...
		p.Boundary = "enmime-" + qbc.Rnd.Uuid()
	}
	if p.ContentID != "" {
		p.Header.Set(hnContentID, utils.ToIDHeader(p.ContentID))
	}
	if p.ContentType != "" {
		...
		setParamValue(param, hpName, utils.ToASCII(p.FileName))
	...
	if p.Disposition != "" {
		...
		param := make(map[string]string)
		setParamValue(param, hpFilename, utils.ToASCII(p.FileName))
