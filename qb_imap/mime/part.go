...
import (
	...
	"github.com/rskvp/qb-core/qb_coding"
...
func (p *Part) setupHeaders(r *bufio.Reader, defaultContentType string) error {
	...
	p.ContentID = utils.FromIDHeader(header.Get(hnContentID))
	return nil
}
...
func (p *Part) convertFromDetectedCharset(r io.Reader) (io.Reader, error) {
	...
	if err != nil {
		return nil, qbc.Errors.WithStack(err)
	}
	...
	if reader, err := utils.NewCharsetReader(cs.Charset, r); err == nil {
		r = reader
...
func (p *Part) convertFromStatedCharset(r io.Reader) io.Reader {
	...
	reader, err := utils.NewCharsetReader(p.Charset, r)
	if err != nil {
...
func (p *Part) decodeContent(r io.Reader) error {
	...
	if err != nil {
		return p.base64CorruptInputCheck(qbc.Errors.WithStack(err))
	}
	p.Content = content
...
func (p *Part) base64CorruptInputCheck(err error) error {
	switch qbc.Errors.Cause(err).(type) {
...
func parseParts(parent *Part, reader *bufio.Reader) error {
	...
	for indexPartID := 1; true; indexPartID++ {
		next, err := br.Next()
		if err != nil && qbc.Errors.Cause(err) != io.EOF {
			return err
		}
		if br.unbounded {
	...
	if err != nil {
		return qbc.Errors.WithStack(err)
	}
