...
func (b *boundaryReader) Read(dest []byte) (n int, err error) {
	...
	for i := 0; i < len(dest); i++ {
		...
		if err != nil && err != io.EOF {
			return 0, qbc.Errors.WithStack(err)
		}
	...
	for {
		...
		for {
			...
			if err == nil || err == io.EOF {
				...
				return false, qbc.Errors.WithStack(err)
			}
		}
		...
		b.finished = true
		return false, qbc.Errors.WithMessagef(errNoBoundaryTerminator, "expecting boundary %q, got %q", string(b.prefix), string(line))
	}
}
